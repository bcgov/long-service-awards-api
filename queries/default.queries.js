...
const queries = {
  findByFields: (fields, values, schema, sort) => {
    ...
    const orderClause =
      order && orderby ? `ORDER BY ${orderby} ${order || "ASC"}` : "";

    // construct where condition sql
    const where = fields
      .map((field, index) => {
        return `"${field}" = $${index + 1}::${
          schema.attributes[field].dataType
        }`;
      })
      .join(" AND ");
  ...
  upsert: (data, schema, conflict = ["id"]) => {
    ...
    let sql = `
            INSERT INTO ${schema.modelName} ("${columns.join(
      '", "'
    )}") VALUES (${values.join(", ")})
  ...
  update: (data, schema, ignore = ["id", "created_at"], idKey = "id") => {
    ...
    if (!schema.modelName || !data || Object.keys(data).length === 0)
      return null;
...
const attachReferences = async (result, schema, idKey = "id") => {
  ...
  await Promise.all(
    Object.keys(schema.attributes || {}).map(async (attKey) => {
  ...
  await Promise.all(
    Object.keys(schema.attachments || {}).map(async (refKey) => {
      const refData = await schema.attachments[refKey].get(parentID);
      // check if reference data is an array or single model instance
      if (refData)
        result[refKey] = Array.isArray(refData)
          ? refData.map((refDataItem) => {
              return refDataItem.data;
            })
          : refData.data;
    })
  );
